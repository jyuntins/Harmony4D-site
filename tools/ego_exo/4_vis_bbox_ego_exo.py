...
def main():
    parser = argparse.ArgumentParser(description='Visualization of bboxes of sequence.')
    ...
    vis_output_path = os.path.join(args.output_path, 'vis_bbox') 
    ...
    for t in tqdm(time_stamps):
        ...
        for (camera_name, camera_mode) in tqdm(cameras):
            ...
            if camera_name in ['aria01', 'aria02']:
                bboxes = np.load(sequence_path + '/processed_data/bbox/{}/{}/{:05d}.npy'.format(camera_name, camera_mode, t), allow_pickle=True).item()
            else:
                bboxes = np.load(sequence_path + '/processed_data/bbox/{}/{:05d}.npy'.format(camera_name, t), allow_pickle=True).item()
