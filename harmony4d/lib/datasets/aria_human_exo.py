...
class AriaHuman:
    def __init__(self, cfg, root_dir, human_name, human_id=0, ground_plane=None, coordinate_transform=None, color=[255, 0, 0], alpha=0.7):
        ...
        self.calibration_path = os.path.join(self.root_dir, 'cam01', 'images')
        ...
        self.images_path = os.path.join(self.root_dir, 'cam01', 'images')
    ...
    ##--------------------------------------------------------
    def get_refine_poses3d(self, max_time_stamp=1200):

        total_time = len(self.poses3d_trajectory)

        ## divide the total time into chunks of max_time_stamp
        num_chunks = total_time // max_time_stamp
        if total_time % max_time_stamp != 0:
            num_chunks += 1
        
        refine_poses3d_trajectory = []
        for chunk_idx in range(num_chunks):
            start_time = chunk_idx*max_time_stamp
            end_time = (chunk_idx + 1)*max_time_stamp
            if end_time > total_time:
                end_time = total_time

            ## print in cyan color start_time and end_time
            print('\033[96m' + 'start_time: {}, end_time: {}'.format(start_time, end_time) + '\033[0m')
            refine_poses3d_trajectory_chunk = refine_pose3d(cfg=self.cfg, human_name=self.human_name, poses=self.poses3d_trajectory[start_time:end_time]) ## max_time_stamp x 17 x 4
            refine_poses3d_trajectory.append(refine_poses3d_trajectory_chunk)

        self.refine_poses3d_trajectory = np.concatenate(refine_poses3d_trajectory, axis=0) ## T x 17 x 4
        assert(self.refine_poses3d_trajectory.shape[0] == total_time)

        return self.refine_poses3d_trajectory    

    ##--------------------------------------------------------
    def fit_poses3d(self, max_time_stamp=1200):
        ...
        for chunk_idx in range(num_chunks):
            ...
            ## print in cyan color start_time and end_time
            print('\033[96m' + 'start_time: {}, end_time: {}'.format(start_time, end_time) + '\033[0m')
            fit_poses3d_trajectory_chunk = fit_pose3d(cfg=self.cfg, human_name=self.human_name, poses_numpy=self.poses3d_trajectory[start_time:end_time])
            fit_poses3d_trajectory.append(fit_poses3d_trajectory_chunk)

        self.fit_poses3d_trajectory = np.concatenate(fit_poses3d_trajectory, axis=0) ## T x 17 x 4
        assert(self.fit_poses3d_trajectory.shape[0] == total_time)

        return self.fit_poses3d_trajectory    