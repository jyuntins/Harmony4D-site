...
from .exo_camera_demo import ExoCameraDemo
...
class EgoExoScene:
    def __init__(self, cfg, root_dir):
        ...
        if "demo_tennis" in root_dir: 
            print("[Warning] Skipping colmap for demo_tennis")
        ...
        ##------------------------exo--------------------------
        if "demo_tennis" in root_dir:
            print("[Warning] Loading exo cameras using ExoCameraDemo class")
            self.exo_camera_names = ["cam01", "cam02", "cam03", "cam04"]
            self.exo_cameras = {
                exo_camera_name: ExoCameraDemo(root_dir=root_dir, exo_camera_name=exo_camera_name) 
    ...
    def init_fit_pose3d(self):
        ...
        for time_stamp in time_stamps:
            ...
            if "demo_tennis" in self.refine_pose3d_dir:
                poses3d = np.load(pose3d_path)
                poses3d_trajectory["aria01"].append(poses3d.reshape(1, -1, 4)) ## 1 x 17 x 4
            else:
                poses3d = (np.load(pose3d_path, allow_pickle=True)).item()
    ...
    def cam_smpl(self, smpl, save_path, draw=False):
        ...
        for human_name in smpl.keys():
            ...
            ariafromcolmap = self.inv_colmap_from_aria_transforms.copy()
            # scale = np.linalg.norm(ariafromcolmap['aria01'][0, :3])
    ...
    def draw_smpl_2d_joints(self, smpl, save_path):
        ...
        return 
    ##--------------------------------------------------------
    def draw_smpl_pare_blender(self, smpl, mesh_save_dir, save_path, save_white=False):
        ...
        ##--------save the mask------------------
        if save_white == True:
            white_canvas = 255*np.ones(image.shape)
            image = mask*render_image + (1-mask)*white_canvas
            save_path = save_path.replace('.jpg', '_white.jpg')
            cv2.imwrite(save_path, image)
    ...
    def get_gopros_plane(self):
        gopros_location = np.array([self.exo_cameras[exo_camera_name].get_location() for exo_camera_name in self.exo_cameras.keys()])
        points_mean = gopros_location.mean(axis=0)
        # points_centered = points - points_mean
        
        # Use SVD to find the best fitting plane
        _, _, vh = np.linalg.svd(gopros_location)
        
        # The plane's normal vector is the last row of vh and corresponds to the smallest singular value
        normal_vector = vh[-1, :]
        A, B, C = normal_vector
        
        # To find D, we use the plane equation with the mean point
        D = -np.dot(normal_vector, points_mean)

        return (A, B, C, D), points_mean
    ...
    def get_bboxes(self):
        ...
        for aria_human in aria_humans:
            aria_human.gopros_plane, aria_human.gopros_mean = self.get_gopros_plane()
