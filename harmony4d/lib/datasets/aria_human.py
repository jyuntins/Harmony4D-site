...
class AriaHuman:
    def __init__(self, cfg, root_dir, human_name, human_id=0, ground_plane=None, coordinate_transform=None, color=[255, 0, 0], alpha=0.7):
        ...
        self.gopros_plane = None
        self.gopros_mean = None
    ...
    def get_cylinder_mesh(self, point_3d, padding=1.1):
        ...
        if self.cfg.BBOX.HUMAN_HEIGHT is None:
            ...
            # if np.dot(self.unit_normal, (point_3d_ - self.point_on_plane)) > 0:
                ...
                # human_height = -1*human_height
            
            cylinder_center = point_3d - human_height*self.unit_normal*0.5
            if cylinder_center[2] > point_3d[2]:
                cylinder_center = point_3d - human_height*self.unit_normal*0.5 * (-1)

            projected_point, distance_to_ground = projected_point_to_plane(self.gopros_mean, self.ground_plane)
                
            # print(self.unit_normal)
            # print(self.ground_plane.get_equation())
            # print(self.gopros_plane)
            # print("mean", self.gopros_mean)
            # print("projected", projected_point)
            # print((self.gopros_mean - projected_point))
            # print(np.dot((self.gopros_mean - projected_point), self.unit_normal))
            # print()
            # cylinder_center = point_3d - human_height*self.unit_normal*0.5 
            # if np.dot((self.gopros_mean - projected_point), self.unit_normal) < 0: #if the direction from the ground to the gopro plane is opposite to the normal unit of the ground plane, reverse the normal unit  
            #     cylinder_center = point_3d - human_height*self.unit_normal*0.5*(-1) 
