...
from models.center_pose_estimator import CenterPoseModel
from models.seg_pose_estimator import SegPoseModel
from models.segmentator import SegmentationModel
from models.human_detector import DetectorModel

from .aria_human_exo import AriaHuman
...
class EgoExoScene:
    def __init__(self, cfg, root_dir):
        ...
        self.fit_pose3d_metric_dir = os.path.join(self.root_dir, 'processed_data', 'fit_poses3d_metric')
        ...
        self.primary_transform = np.eye(4)
        self.coordinate_metric_transform = np.eye(4) ## To DO: not used right now!!
        self.coordinate_metric = False

        ## if colmap_dir, scale.npy exists, then we load it to primary_transform

        if os.path.exists(os.path.join(self.colmap_dir, 'scale.npy')) and self.cfg.CALIBRATION.LOAD_METRIC_SCALE_TRANSFORM == True:
            T = np.load(os.path.join(self.colmap_dir, 'scale.npy'))
            self.primary_transform = np.linalg.inv(T)
            # print(self.primary_transform)
            # input()
            # self.primary_transform = T

            self.coordinate_metric = True
        ...
        for person_idx, aria_human_name in enumerate(self.aria_human_names):
            ...
            self.aria_humans[aria_human_name] = AriaHuman(
                            cfg=cfg,
                            root_dir=self.exo_dir, human_name=aria_human_name, \
        ...
        ## total_time is number of .jpg files in exo/cam01/images
        ## if cam01 does not exist, then we use cam02 and so on
        self.total_time = len([file for file in os.listdir(os.path.join(self.exo_dir, self.exo_camera_names[0], 'images')) if file.endswith('jpg')])
        ...
        for aria_human_name in self.aria_human_names:
            self.aria_humans[aria_human_name].total_time = self.total_time
        ...
        self.camera_names = self.exo_camera_names_with_mode ##[(camera_name, camera_mode)...]
        ...
        self.load_contact_pose3d_flag = False
        self.load_refine_pose3d_flag = False
        self.load_fit_pose3d_flag = False
        self.load_fit_pose3d_metric_flag = False
    ...
    def load_scene_geometry(self, max_dist=0.1):
        ...
        print('\033[96m' + 'The ground plane equation ax+by+cz+d = 0, [a,b,c,d]: {}'.format([a, b, c, d]) + '\033[0m')
    ...
    ##--------------------------------------------------------
    def init_smpl(self):
        self.load_fit_pose3d_metric_flag = True
        self.total_time_fit_pose3d_metric = len([file for file in os.listdir(self.fit_pose3d_metric_dir) if file.endswith('npy')])
    ...
    def init_smpl_trajectory(self):
        ...
        ##----temporal-----
        self.total_time_fit_pose3d_metric = len([file for file in os.listdir(self.fit_pose3d_metric_dir) if file.endswith('npy')])
        time_stamps = list(range(1, self.total_time_fit_pose3d_metric + 1))
        poses3d_trajectory = {aria_human_name:[] for aria_human_name in self.aria_human_names}

        for time_stamp in time_stamps:
            pose3d_path = os.path.join(self.fit_pose3d_metric_dir, '{:05d}.npy'.format(time_stamp))
    ...
    def load_fit_pose3d_metric(self):
        self.load_fit_pose3d_metric_flag = True
        self.total_time_fit_pose3d_metric = len([file for file in os.listdir(self.fit_pose3d_metric_dir) if file.endswith('npy')])
        return
    ...
    def load_contact_pose3d(self):
        self.load_contact_pose3d_flag = True
        self.total_time_contact_pose3d = len([file for file in os.listdir(self.contact_pose3d_dir) if file.endswith('npy')])
        return
    ...
    def init_pose2d_rgb(self, dummy=False):
        ...
        if dummy == True:
            ...
            rgb_pose_config = self.cfg.POSE2D.RGB_CONFIG_FILE # HRNET 
            rgb_pose_checkpoint = self.cfg.POSE2D.RGB_CHECKPOINT # HRNET checkpoint

        self.rgb_pose_model = PoseModel(cfg=self.cfg, pose_config=rgb_pose_config, pose_checkpoint=rgb_pose_checkpoint) # create HRNET model
    ...
    def update(self, time_stamp):
        ...
        if self.load_pose3d_flag == True:
            ...
            if self.cfg.POSE3D.USE_SEGPOSE2D == True:
        ...
        if self.load_contact_pose3d_flag == True:
            pose3d_path = os.path.join(self.contact_pose3d_dir, '{:05d}.npy'.format(time_stamp))
            pose3d = np.load(pose3d_path, allow_pickle=True)
        ...
        ## load fitted 3d poses metric
        if self.load_fit_pose3d_metric_flag == True:
            pose3d_path = os.path.join(self.fit_pose3d_metric_dir, '{:05d}.npy'.format(time_stamp))
            pose3d = np.load(pose3d_path, allow_pickle=True)
            self.set_poses3d(pose3d.item())
    ...
    def match_smpl_shape(self):
        smpl_params_trajectory = {}
        initial_smpl_trajectory = self.load_initial_smpl_trajectory() ## H x T x smpl_info, nested dicts

        for human_name in self.aria_human_names:
            poses3d_trajectory = self.aria_humans[human_name].poses3d_trajectory ## T x 17 x 4
            smpl_params_trajectory[human_name] = self.smplify.shape_matching(human_name=human_name, poses3d_trajectory=poses3d_trajectory, \
                    initial_smpl_trajectory=initial_smpl_trajectory[human_name])

        return smpl_params_trajectory
    
    def optimize_with_betas_freezed(self):
        smpl_params_trajectory = {}
        initial_smpl_trajectory = self.load_initial_smpl_trajectory() ## H x T x smpl_info, nested dicts

        for human_name in self.aria_human_names:
            poses3d_trajectory = self.aria_humans[human_name].poses3d_trajectory ## T x 17 x 4
            smpl_params_trajectory[human_name] = self.smplify.freeze_betas(human_name=human_name, poses3d_trajectory=poses3d_trajectory, \
                    initial_smpl_trajectory=initial_smpl_trajectory[human_name])
    ...
    def get_initial_smpl(self, choosen_camera_names, bbox_padding=1.25):
        ...
        for camera in choosen_cameras:
            ...
            # np.save('/media/rawalk/disk1/rawalk/datasets/ego_exo/main/19_sword2/002_sword2/exo/cam01/images/test/cliff.npy', initial_smpl, allow_pickle=True)
            # breakpoint()
            # for human_name in initial_smpl.keys():
            #     print(camera, human_name, "initial height", np.max(initial_smpl[human_name]['vertices'][:, 1]) - np.min(initial_smpl[human_name]['vertices'][:, 1]))
            # scale_temp = np.load('/media/rawalk/disk1/rawalk/datasets/ego_exo/main/30_karate3/011_karate3/colmap/workplace/scale_temp.npy', allow_pickle=True)
            # print(scale_temp)
            # print(scale_temp.shape)
            # input()
            # for human_name in initial_smpl.keys():
            #     initial_smpl[human_name]["vertices"] = np.dot(np.linalg.inv(scale_temp), np.vstack((initial_smpl[human_name]["vertices"].T, np.ones((1, 6890))))).T[:, :3]

            # image = cv2.imread(image_path)
            # print(bboxes)
            # bbox_2d = bboxes['aria01']
            # color = np.array([255,0,0])

            # thickness = 12
            # print(bbox_2d[0], bbox_2d[1], bbox_2d[2], bbox_2d[3])
            # image = cv2.rectangle(image, (int(bbox_2d[0]), int(bbox_2d[1])), (int(bbox_2d[2]), int(bbox_2d[3])), thickness)
            # image = cv2.resize(image, (1920, 1080))
            # cv2.imshow('image', image)
            # cv2.waitKey(0)
            # # print(initial_smpl)
            # print(np.max(initial_smpl['aria02']['vertices'][:, 1]))
            # print(np.min(initial_smpl['aria02']['vertices'][:, 1]))
            # print("initial height", np.max(initial_smpl['aria02']['vertices'][:, 1]) - np.min(initial_smpl['aria02']['vertices'][:, 1]))
            # # print(type(initial_smpl))
            # # print(initial_smpl.keys())
            # # print(len(initial_smpl['aria01']['joints']))#'betas', 'rotmat', 'pose', 'cam_full', 'vertices', 'joints', 'focal_length'
            # input()
            ...
            # print(np.max(best_initial_smpl['aria02']['transformed_vertices'][:, 1]))
            # print(np.min(best_initial_smpl['aria02']['transformed_vertices'][:, 1]))
            # input(np.max(best_initial_smpl['aria02']['transformed_vertices'][:, 1]) - np.min(best_initial_smpl['aria01']['transformed_vertices'][:, 1]))
        ...
        # print(best_initial_smpl['aria01'].keys())
        # input()
        np.save('/home/rawalk/Desktop/ego/ego_exo/scripts/11_song/best_init_smpl.npy', best_initial_smpl)
    ...
    def get_initial_smpl_transformation(self, camera, pose3d, smpl, all_pose2d, exo_cameras, error_thres=1.0, \
        ...
        # from scipy.spatial.transform import Rotation as R
        # rotation_matrix = R.from_euler('xyz', init_global_orient, degrees=True).as_matrix()
        # transformation_matrix = np.eye(4)
        # transformation_matrix[:3, :3] = rotation_matrix
        # transformed_vertices = np.dot(transformation_matrix, np.vstack((init_vertices.T, np.ones((1, 6890))))).T[:, :3]
        # transformed_vertices = transformed_vertices + init_transl
        # scale_temp = np.load('/media/rawalk/disk1/rawalk/datasets/ego_exo/main/30_karate3/011_karate3/colmap/workplace/scale_temp.npy', allow_pickle=True)
        # transformed_vertices = np.dot(np.linalg.inv(scale_temp), np.vstack((transformed_vertices.T, np.ones((1, 6890))))).T[:, :3]
        # import torch
        ...
        # global_orient_aa = torch.tensor(init_global_orient.reshape(1, -1), dtype=torch.float32, device=device)
        # transl = torch.tensor(init_transl.reshape(1, -1), dtype=torch.float32, device=device)

        # output = self.smpl_model(vertices=torch.tensor(init_vertices.reshape(1, -1, 3), dtype=torch.float32, device=device),
    ...
    def draw_initial_smpl(self, smpl, save_path):
        ...
        for human_name in smpl.keys():
            ...
            smpl_human = smpl[human_name] # dict_keys(['betas', 'rotmat', 'pose', 'cam_full', 'vertices', 'joints', 'focal_length', 'init_transl', 'init_global_orient', 'transformed_vertices', 'bbox', 'best_view'])
            color = self.aria_humans[human_name].color
            points_3d = smpl_human['transformed_vertices'] ## 6890 x 3

            # root = smpl_human['joints'][0]
            # scale_matrix = np.linalg.inv(np.array([[1.088, 0, 0], [0, 1.088, 0], [0, 0, 1.088]]))
            # points_3d = points_3d - root ## center the mesh to the root joint
            # points_3d = np.dot(scale_matrix, points_3d.T).T
            # points_3d = points_3d + root ## center the mesh to the root joint
            # points_3d = smpl_human['vertices'] ## 6890 x 3
            # points_3d[:, 1] = -points_3d[:, 1] ## flip the y axis
            # scale_temp = np.load('/media/rawalk/disk1/rawalk/datasets/ego_exo/main/30_karate3/011_karate3/colmap/workplace/scale_temp.npy', allow_pickle=True)
            # self.view_camera.extrinsics[:, 3] = np.dot(np.linalg.inv(scale_temp), self.view_camera.extrinsics[:, 3])

            # points_3d = np.dot(np.linalg.inv(scale_temp[:3, :3]), points_3d.T).T
            # points_3d = self.aria_humans[human_name].pose3d[:, :3] 
            points_2d = self.view_camera.vec_project(points_3d)# original!!!!!!!!!!!!!!!!!!!11
            # reciprocal = self.view_camera.coordinate_transform
            # reciprocal[1][1] = 1/reciprocal[1][1]
            # reciprocal[2][2] = 1/reciprocal[2][2]
            # reciprocal[3][3] = 1/reciprocal[3][3]
            # print(reciprocal)
            # input()
            # self.view_camera.extrinsics = np.dot(reciprocal, self.view_camera.extrinsics)
            # point_3d_cam = self.view_camera.vec_cam_from_world(points_3d)
            # points_2d = self.view_camera.vec_image_from_cam(point_3d_cam)
    ...
    def init_cam_smpl_model(self):
        self.smpl_model = SMPLModel(cfg=self.cfg)
        return

    def cam_smpl(self, smpl, save_path, draw=False):
        if draw:
            original_image = self.view_camera.get_image(time_stamp=self.time_stamp)
            image = original_image.copy()
            overlay = 255*np.ones(image.shape)
        ...
        for human_name in smpl.keys():
            ...
            global_orient = smpl_human['global_orient']
            cam_translation = smpl_human['transl']
            if draw:
                color = self.aria_humans[human_name].color
                body_pose = smpl_human['body_pose']
                shape_param = smpl_human['betas']

            global_orient = global_orient.reshape(1, 3)
            rotation = aa_to_rotmat(global_orient)

            smpl_rotation = np.identity(4)
            smpl_rotation[:3, :3] = rotation
            smpl_translation = np.identity(4)
            smpl_translation[:3, 3] = cam_translation

            extrinsics = self.view_camera.extrinsics.copy()

            scale = 1/np.linalg.norm(extrinsics[0, :3])
            # scale = 1/np.linalg.norm(self.colmap_from_aria_transforms['aria01'][0, :3])
            scale_matrix = scale*np.identity(4)
            scale_matrix[3,3]=1

            T = scale_matrix @ extrinsics @ smpl_translation @ smpl_rotation
            # T = extrinsics @ smpl_translation @ smpl_rotation

            translation_vector = T[:3, 3]

            cam_rotation = T[:3, :3].copy()
            cam_translation = T[:3, 3].copy()

            cam_rotation_aa = rotmat_to_aa(matrix=cam_rotation)
            cam_global_orient[human_name] = cam_rotation_aa

            if draw:
                transformed_vertices = self.smpl_model.get_initial_vertices(betas=shape_param, \
                                body_pose_aa=body_pose, global_orient_aa=cam_rotation_aa, transl=cam_translation)
                # cam space to image space only
                points_2d = self.view_camera.vec_image_from_cam(transformed_vertices)
        ...
        if draw:
            ...
            image = np.concatenate([image, overlay], axis=1)
            cv2.imwrite(save_path, image)
        return cam_global_orient
    
    def draw_smpl_2d_joints(self, smpl, save_path):
        original_image = self.view_camera.get_image(time_stamp=self.time_stamp)
        image = original_image.copy()
        overlay = 255*np.ones(image.shape)
        alpha = 0.7
    ...
    def triangulate(self, flag='exo', secondary_flag='ego_rgb', debug=False, pose2d=None):
        ...
        #     pose3d = poses3d[human_name]
        #     transformed_pose3d = (self.coordinate_metric_transform @ pose3d.T).T
        #     poses3d[human_name] = transformed_pose3d
    ...
    def load_bboxes(self):
        ...
        if os.path.exists(bboxes_path):
            ...
            for human_name in self.aria_human_names:
                if human_name == 'aria01':
                    bbox = {
                            'bbox':np.array([100, 100, 200, 300, 1.0]), \
                            'human_name': 'aria01', \
                            'human_id': 1, \
                            'color': [255, 0, 0], \
                        }
                ...
                elif human_name == 'aria02':
                    bbox = {
                            'bbox':np.array([200, 300, 400, 500, 1.0]), \
                            'human_name': 'aria02', \
                            'human_id': 2, \
                            'color': [0, 255, 0], \
                        }
    ...
    def draw_smpl_bboxes(self, bboxes, save_path): 
        # This function is only used for drawing bboxes generated by smpl mask.
        # TODO: Merge this function with the previous draw_bboxes. 
        image_name = self.view_camera.get_image_path(time_stamp=self.time_stamp)
        image = cv2.imread(image_name)
        color = (0, 0, 255)
