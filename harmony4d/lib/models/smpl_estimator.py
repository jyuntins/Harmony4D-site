...
cliff_dir = os.path.join(pathlib.Path(__file__).parent.resolve(), '..', '..', '..', 'cliff') 
add_path(cliff_dir)
...
def get_smpl_faces():
    try:
        ...
        print('environment error, loading the smpl faces locally')
        assets_dir = os.path.join(pathlib.Path(__file__).parent.resolve(), '..', '..', '..', 'ego_exo', 'assets') 
...
class SMPLModel:
    ...
    def load_model(self):
        ...
        # cliff_model = cliff_res50(constants.SMPL_MEAN_PARAMS).to(self.device)
        ckpt_path = os.path.join(cliff_dir, 'data/ckpt/hr48-PA43.0_MJE69.0_MVE81.2_3dpw.pt')
        # ckpt_path = os.path.join(cliff_dir, 'data/ckpt/res50-PA45.7_MJE72.0_MVE85.3_3dpw.pt')
    ...
    def get_initial_smpl(self, image_path, bboxes, bbox_padding=1.25):
        ...
        for batch in tqdm(mocap_data_loader):
            ...
            # np.save('/media/rawalk/disk1/rawalk/datasets/ego_exo/main/19_sword2/002_sword2/exo/cam01/images/test/vertice2.npy', pred_vertices, allow_pickle=True)
            # breakpoint()
