...
class SMPLify:
    def __init__(self, cfg):
        ...
        self.shape_matching_config_file = os.path.join(mmhuman3d_dir, 'configs', 'smplify', 'shape_matching.py')
        self.original_shape_matching_config = mmcv.Config.fromfile(self.shape_matching_config_file)

        self.freeze_betas_config_file = os.path.join(mmhuman3d_dir, 'configs', 'smplify', 'freeze_betas.py')
        self.original_freeze_betas_config = mmcv.Config.fromfile(self.freeze_betas_config_file)
        return
    ...
    def build_smplify_shape_matching(self, human_name):
        all_human_names = self.cfg.SMPL.ARIA_NAME_LIST
        idx = all_human_names.index(human_name)
        num_epochs = self.cfg.SMPL.NUM_EPOCHS_LIST[idx]
        stage1_iters = self.cfg.SMPL.STAGE1_ITERS_LIST[idx]
        ...
        ##---------build-------------
        shape_matching_config = self.original_shape_matching_config.copy()
        shape_matching_config.update(dict(
                            verbose=self.cfg.SMPL.VERBOSE,
                            body_model=self.body_model_config,
                            use_one_betas_per_video=True,
                            num_epochs=num_epochs))
        shape_matching_config['stages'][0]['num_iter'] = stage1_iters
        shape_matching_config['stages'][1]['num_iter'] = stage2_iters
        shape_matching_config['stages'][2]['num_iter'] = stage3_iters

        shape_matching = build_registrant(dict(shape_matching_config))

        return shape_matching, shape_matching_config
    
    def build_smplify_freeze_betas(self, human_name):
        all_human_names = self.cfg.SMPL.ARIA_NAME_LIST
        idx = all_human_names.index(human_name)
        num_epochs = self.cfg.SMPL.NUM_EPOCHS_LIST[idx]
        stage1_iters = self.cfg.SMPL.STAGE1_ITERS_LIST[idx]
        ...
        ##---------build-------------
        freeze_betas_config = self.original_freeze_betas_config.copy()
        freeze_betas_config.update(dict(
                            verbose=self.cfg.SMPL.VERBOSE,
                            body_model=self.body_model_config,
                            use_one_betas_per_video=True,
                            num_epochs=num_epochs))
        freeze_betas_config['stages'][0]['num_iter'] = stage1_iters
        freeze_betas_config['stages'][1]['num_iter'] = stage2_iters
        freeze_betas_config['stages'][2]['num_iter'] = stage3_iters

        freeze_betas = build_registrant(dict(freeze_betas_config))

        return freeze_betas, freeze_betas_config
    ...
    def cluster_beta(self, beta, min_cluster_size=3):
        ...
        clusterer = hdbscan.HDBSCAN(min_cluster_size=2) #int(min_cluster_size)) # Adjust this value based on your data
    ...
    def shape_matching(self, human_name, poses3d_trajectory, initial_smpl_trajectory, skip_face=False):
        """
        all_poses3d_trajectory is time x 17 x 4
        initial_smpl_trajectory is time x smpl_info --> nested dicts
        """
        ...
        self.smplify, self.smplify_config = self.build_smplify_shape_matching(human_name)
    ...
    def freeze_betas(self, human_name, poses3d_trajectory, initial_smpl_trajectory, skip_face=False):
        """
        all_poses3d_trajectory is time x 17 x 4
        initial_smpl_trajectory is time x smpl_info --> nested dicts
        """
        ...
        self.smplify, self.smplify_config = self.build_smplify_freeze_betas(human_name)
