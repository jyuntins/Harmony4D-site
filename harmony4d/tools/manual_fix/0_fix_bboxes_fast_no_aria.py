...
def main():
    ...
    camera_names = sorted(scene.exo_cameras.keys())

    camera_mode = 'rgb'
    factor = 3.0 ## resize factor, the original image is too big

    ## assert that the timesteps are starting from 1 and are consecutive
    # assert(time_stamps[0] == 1)
    for i in range(len(time_stamps)-1):
        assert(time_stamps[i+1] == time_stamps[i]+1)
    
    manual_time_stamps = [time_stamps[0]]*len(camera_names)

    for t, camera_name in tqdm(zip(manual_time_stamps, camera_names)):
        ...
        ## save and vis for all the time stamps
        save_dir = os.path.join(output_path, scene.viewer_name, scene.view_camera_type)
        os.makedirs(save_dir, exist_ok=True)
        save_vis_dir = os.path.join(vis_output_path, scene.viewer_name, scene.view_camera_type)
        os.makedirs(save_vis_dir, exist_ok=True)

        for t_temp in time_stamps:
            save_path = os.path.join(save_dir, '{:05d}.npy'.format(t_temp))
            scene.save_bboxes(bboxes, save_path)
        
            ##-------------visualize the bboxes-------------------            
            save_path = os.path.join(save_vis_dir, '{:05d}.jpg'.format(t_temp))
