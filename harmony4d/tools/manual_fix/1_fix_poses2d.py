...
def main():
    ...
    parser.add_argument('--cameras', default='cam01', help='camera_names')
    ...
    output_path = os.path.join(args.output_path, 'poses2d') 
    ...
    for t, camera_name in tqdm(zip(time_stamps, camera_names)):
        ...
        poses2d = scene.load_poses2d() ## list of humans

        poses2d_resized = poses2d.copy()
        for i in range(len(poses2d_resized)):
            poses2d_resized[i]['keypoints'] = poses2d_resized[i]['keypoints'][:17].copy()
            poses2d_resized[i]['keypoints'][:, 0] = poses2d_resized[i]['keypoints'][:, 0] / factor ## resize by factor, only 17 keypoints 
            poses2d_resized[i]['keypoints'][:, 1] = poses2d_resized[i]['keypoints'][:, 1] / factor ## resize by factor, only 17 keypoints 
        ...
        def click_event(event, x, y, flags, params):
            ...
            if params['human_name'] is not None:
                idx = find_human_idx(params['human_name'], params['poses2d'])
                bbox_info = params['poses2d'][idx]['bbox']
                color = params['poses2d'][idx]['color']

                x1 = int(bbox_info[0] / factor)
                y1 = int(bbox_info[1] / factor)
                x2 = int(bbox_info[2] / factor)
                y2 = int(bbox_info[3] / factor)
                params['canvas'] = cv2.rectangle(params['canvas'], (x1, y1), (x2, y2), color, 3)
        ...
        params = {'poses2d': poses2d_resized, 'human_name': None, \
                'keypoint_idx': None, \
                'is_ongoing': False, \
