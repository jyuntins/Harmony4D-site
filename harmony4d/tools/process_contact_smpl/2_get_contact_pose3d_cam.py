...
def main():
    ...
    parser.add_argument('--num_cameras', default=20, help='number of cameras')
    ...
    output_path = os.path.join(args.output_path, 'contact_poses3d'+str(args.num_cameras)) 
    poses3d_output_path = os.path.join(args.output_path, 'poses3d')
    vis_output_path = os.path.join(args.output_path, 'vis_contact_poses3d'+str(args.num_cameras)) 
    print('sequence at {}'.format(sequence_path))

    os.makedirs(output_path, exist_ok=True)
    os.makedirs(vis_output_path, exist_ok=True)

    vis_segmentation_output_path = os.path.join(args.output_path, 'vis_segmentation'+str(args.num_cameras))
    vis_kf_poses3d_output_path = os.path.join(args.output_path, 'vis_kf_poses3d'+str(args.num_cameras))
    vis_segpose2d_output_path = os.path.join(args.output_path, 'vis_segposes2d'+str(args.num_cameras))
    ...
    for t in tqdm(time_stamps):
        ...
        valid_camera_names = ['cam{:02d}'.format(i + 1) for i in range(num_cameras)]
        ...
        for (camera_name, camera_mode) in cameras:
            if not camera_name in valid_camera_names:
                continue
        ...
        for (camera_name, camera_mode) in cameras:
            ...
            # segmentation = segmentations[(camera_name, camera_mode)]
            # segpose2d = segposes2d[(camera_name, camera_mode)] 
            ...
            # save_dir = os.path.join(vis_segmentation_output_path, scene.viewer_name, scene.view_camera_type)
            # os.makedirs(save_dir, exist_ok=True)
            # save_path = os.path.join(save_dir, '{:05d}.jpg'.format(t))
            # scene.draw_segmentation(segmentation, save_path)

            # ## visualize poses2d from seg
            # save_dir = os.path.join(vis_segpose2d_output_path, scene.viewer_name, scene.view_camera_type)
            # os.makedirs(save_dir, exist_ok=True)
            # save_path = os.path.join(save_dir, '{:05d}.jpg'.format(t))
            # scene.draw_poses2d(segpose2d, save_path)
        
        ##---------------------------------------------------------------------------
        ## also visualize the KF poses
        # scene.set_poses3d(kf_poses3d) ## set the kf_pose3d to humans
        # for (camera_name, camera_mode) in cameras:
        #     scene.set_view(camera_name=camera_name, camera_mode=camera_mode)
        #     poses2d = scene.get_projected_poses3d()    
        ...
        #     if t%10 != 0:
        #         continue
        #     save_dir = os.path.join(vis_kf_poses3d_output_path, scene.viewer_name, scene.view_camera_type)
        #     os.makedirs(save_dir, exist_ok=True)
        #     save_path = os.path.join(save_dir, '{:05d}.jpg'.format(t))
        #     scene.draw_projected_poses3d(poses2d, save_path)
