...
def main():
    ...
    parser.add_argument('--vis', action='store_true', help='number of workers')
    ...
    output_path = os.path.join(args.output_path, 'cam_smpl') 
    vis_output_path = os.path.join(args.output_path, 'vis_cam_smpl') 
    ...
    for t in tqdm(time_stamps):
        ...
        for (camera_name, camera_mode) in cameras:
            ...
            if camera_name in ['aria01', 'aria02', 'cam21', 'cam22']:
                continue
            ...
            vis_save_dir = os.path.join(vis_output_path, scene.viewer_name, scene.view_camera_type)
            os.makedirs(vis_save_dir, exist_ok=True)
            vis_save_path = os.path.join(vis_save_dir, '{:05d}.jpg'.format(t))
            cam_global_orient = scene.cam_smpl(smpl, vis_save_path, draw=args.vis)

            save_dir = os.path.join(output_path, scene.viewer_name, scene.view_camera_type)
            os.makedirs(save_dir, exist_ok=True)
            for human_name in scene.aria_human_names:
                smpl[human_name]['global_orient'] = cam_global_orient[human_name]
