...
def process_camera_view(args):
    scene, camera_name, camera_mode, t, poses3d, vis_output_path = args

    # Skip cameras if not in the VIS_CAMERAS list
    if scene.cfg.REFINE_POSE3D.VIS_CAMERAS != [] and camera_name not in scene.cfg.REFINE_POSE3D.VIS_CAMERAS:
        return
...
def main():
    ...
    parser.add_argument('--additional_scale_factor', default=1.0, help='additional scale factor to be applied to the 3d poses')
    ...
    output_path = os.path.join(args.output_path, 'refine_poses3d') 
    vis_output_path = os.path.join(args.output_path, 'vis_refine_poses3d') 
    ...
    ##-----------------poses 3d-----------------------
    scene.init_refine_pose3d() ## load all the 3d poses in memory
    scene.init_pose2d_rgb() ## for visualization

    poses3d_trajectory = scene.get_refine_poses3d() # human x time x pose3d
    ...
    print('loaded {} poses3d'.format(len(poses3d_trajectory.keys())))
    for human_name in poses3d_trajectory.keys():
        print('human {} has {} poses3d'.format(human_name, len(poses3d_trajectory[human_name])))
    ...
    for t in tqdm(time_stamps):
        ...
        ## print height of the humans, distance between nose and left ankle
        for human_name in poses3d.keys():
            pose3d = poses3d[human_name]
            nose = pose3d[0]
            left_ankle = pose3d[15]
            
            ## height is the euclidean distance between nose and left ankle
            height = ((nose[0] - left_ankle[0])**2 + (nose[1] - left_ankle[1])**2 + (nose[2] - left_ankle[2])**2)**0.5
            ...
            print('time: {}, human: {}, height: {} feet {} inches'.format(t, human_name, height_feet, height_inches_remainder))
